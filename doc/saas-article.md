# 单机部署产品 SaaS 化改造实录：多租户数据隔离与那些绕不开的坑

> 一个"AI 智能获客助手"从单机部署模式改造为开放注册 SaaS 系统的完整技术复盘——租户模型怎么建、14 张业务表怎么加租户隔离、JWT 与拦截器怎么配合、定时任务和 MCP 的租户上下文坑，以及 E2E 全链路验证。

## 一、改造背景：为什么单机产品要 SaaS 化

我的上一个项目是一套**本地部署**的 AI 智能获客系统：客户把整套系统部署在自己服务器上使用。

产品跑通后遇到一个现实问题：**交付成本高、触达新客户慢**。每个客户都要走"联系 → 报价 → 指导部署"的流程，而且很多中小团队其实更想要"注册就能用"的轻量体验。

所以决定做一次彻底改造：**从单机部署模式 → 开放注册 + 多租户数据隔离的 SaaS 模式**。本文记录这次改造的技术决策、实现细节和踩过的坑。
...
单机时代采用过一套激活码体系（离线签名签发 + 机器绑定 + Token 配额）做授权管理。SaaS 化后这套体系不再需要，删除比想象中牵扯更多。
...
这里有个容易忽略的细节：**删掉激活码拦截器后，未知 API 访问不再有兜底**，Spring Boot 4 默认对不存在的路径会直接 500。我加了 `NoResourceFoundException → 404` 的全局异常处理器，让未知接口干净地返回 404 而不是 500：
...
- 删除 `Activate.tsx` + `/activate` 路由；登录页激活提示改为「免费注册」链接
- Nav/Dashboard/Help 去掉激活逻辑，清理相关样式
...
**授权体系移除**：带 token 访问 `/api/license` → 404；`/app/activate` 无路由；全页面无激活提示 ✅

**平台 admin**：`admin` 登录成功，租户级接口优雅降级，登录统计正常 ✅

**页面遍历 + DOM 测量**：8 个业务页面全部正常打开、无残留激活文案、无元素溢出/重叠 ✅
...
3. **删除比新增更考验工程能力**。移除一套授权体系不只是删代码，还要处理"没有拦截器兜底后未知接口变 500"这种连锁反应，以及全前端去残留文案。

## 十一、写在最后：开源是底座，定制是服务

做完这次改造，我最大的收获其实不在技术上，而是一次**策略认知的转变：把"产品"当成"底座 + 服务"来思考**——开源/开放是底座，定制是服务。

单机时代，激活码体系的底层逻辑是"防御"——控制分发、限制使用。它天然把产品和用户放在了对立面：用户想用，系统先怀疑你是不是来白嫖的。这种心态最大的代价不是技术复杂度，而是**它同时挡住了真正想用的客户**：试用要申请、部署要指导、激活要流程，每一步都是流失点。

SaaS 化后的逻辑完全不同：**开放注册把门槛降到了零**，任何用户 30 秒就能用上完整功能。这时候你会发现一个有意思的转变——**"敢不敢开放"其实取决于工程能力，而不是商业决策**。只有当数据隔离（tenant_id 贯穿表、JWT、拦截器、Service 层）足够扎实，才敢把注册的大门完全敞开；隔离做不牢，开放就是安全事故。开放和严谨在这里是一体两面。

这个思路延伸到技术分享上也是一样的。以前总觉得核心代码和踩坑经验是"护城河"，藏起来才有竞争力；现在反而觉得，**把架构思路、踩坑过程、工程取舍公开写出来，换来的信任、反馈和连接，远比"藏着"那点信息差值钱**。这篇文章本身，就是一次"开源"——把真实的工程经验开放出来，而不是守着它。

再往深一层想，这个转变其实回答了一个更本质的问题：**"开源是底座，定制是服务"意味着什么？**

1. **底座要"薄"而"准"**。底座（核心引擎、数据模型、隔离框架）是给所有用户共用的通用能力，它必须薄——只解决 80% 的通用问题，不掺入少数客户的定制逻辑；又必须准——每个决策都经得起公开检验。底座做得越干净，定制层的空间就越大。
2. **定制要"厚"而"贴"**。定制（私有化部署、专属流程、数据接入、品牌集成）才是利润和壁垒所在。它贴着每个客户的具体场景走，天然不适合开源——这是服务的价值，也是商业化的落点。用户愿意为"适配我的业务"付费，而不是为"能用"付费。
3. **底座和定制互相成就**。底座开放得越彻底，信任建立得越快，愿意来定制的人越多；定制服务积累的行业经验，又会反哺底座，让它更通用、更稳。开源不是放弃商业，而是**把商业化从"卖底座"移到"卖服务"**——用户因为底座好用而留下，因为定制贴身而付费。这种"靠价值而非锁"的增长，更健康，也更能长久。

回到这次改造本身：租户隔离这套底座做得足够扎实，才敢放开注册；注册放开带来的真实用户反馈，又让底座更快进化——这正是"开源是底座、定制是服务"的一个小小缩影。

如果你也在做 B2B 获客，或者对「单机产品 → SaaS 多租户改造」这类话题感兴趣，这套系统的 AI 潜客挖掘、个性化邮件/微信触达、合规退订管理、登录与邮件数据复盘都值得参考，欢迎一起交流。
