...
- 巡检视口：iPhone SE 375x667 / iPhone 14 390x844 / iPhone 15 Pro Max 430x932 / 超窄 320x568
- 页面主体：四尺寸均通过（无横向溢出、金额按钮/支付按钮/记录列表无重叠、卡片自适应）✅
- **发现并修复**：`.donate-qr-img` 原用 `width:100% + max-height + object-fit:contain`，小屏下图片元素盒子被拉伸（原始比例 0.67 vs 显示 0.83/0.75）→ 改为 `max-width:100%; width:auto; height:auto` 按原图比例自适应
- 修复后复测：三尺寸弹窗通过，支付宝/微信收款码图片均无变形、无溢出、按钮无重叠 ✅
